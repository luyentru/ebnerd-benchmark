...
from helpers import calculate_average_time_difference, map_published_time, calculate_article_age, calculate_time_differences
...
PATH = Path("../../").expanduser() ### <===== CHANGE PATH ACCORDINGLY
...
# global_min = df.select(pl.col("time_differences")).min()[0, 0]
# global_max = df.select(pl.col("time_differences")).max()[0, 0]
...
#     ((pl.col("time_differences") - global_min) / (global_max - global_min))
#     .alias("norm_avg_access_time")
# )

# Flatten the time_differences column to extract all values
flattened_values = np.concatenate(df["time_differences"])

# Compute global_min and global_max for the flattened list
global_min = np.min(flattened_values)
global_max = np.max(flattened_values)
...
def normalize_list(lst, g_min, g_max):
    return [(x - g_min) / (g_max - g_min) for x in lst]

# Apply normalization to each list in the time_differences column
df = df.with_columns(
    pl.col("time_differences").map_elements(
        lambda lst: normalize_list(lst, global_min, global_max),
