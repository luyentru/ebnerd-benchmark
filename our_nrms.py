...
############################################
def print_largest_and_smallest(df, column_name):
    """
    Print the 10 largest and 10 smallest values in a column containing lists of numbers.

    Args:
        df (pl.DataFrame): The DataFrame containing the column.
        column_name (str): The column with lists of numbers.
    """
    # Flatten the lists in the column
    flattened_values = [val for sublist in df[column_name].to_list() for val in sublist]
    
    # Convert to a Polars Series for sorting
    flattened_series = pl.Series(flattened_values)
    
    # Get the smallest and largest values
    smallest_values = flattened_series.sort()[:10].to_list()
    largest_values = flattened_series.sort().reverse()[:10].to_list()

    # Print the results
    print("10 Smallest Values:", smallest_values)
    print("10 Largest Values:", largest_values)

# Use the function on your DataFrame
print_largest_and_smallest(df_train, "time_deltas_normalized")

def count_negative_values(series):
    """
    Counts the number of negative values in a series containing lists of numbers.

    Args:
        series (pl.Series): A Polars Series with lists of numbers.

    Returns:
        int: Total count of negative values.
    """
    return sum(1 for deltas in series if deltas is not None for delta in deltas if delta < 0)

# Count negative values in the 'time_deltas' column
negative_count = count_negative_values(df_train["time_deltas_normalized"])
print(f"Number of negative values in 'time_deltas': {negative_count}")

def filter_users_with_negative_deltas(df, column_name):
    """
    Filters the dataframe to include rows where the specified column contains at least one negative value.

    Args:
        df (pl.DataFrame): The dataframe to filter.
        column_name (str): The column containing lists of time deltas.

    Returns:
        pl.DataFrame: Filtered dataframe with users having negative time deltas.
    """
    return df.filter(
        pl.col(column_name).apply(lambda deltas: any(delta < 0 for delta in deltas if deltas is not None))
    )

# Filter the dataframe
users_with_negative_deltas = filter_users_with_negative_deltas(df_train, "time_deltas_normalized")

# Display the filtered dataframe
print(users_with_negative_deltas)

# Filter the df_articles dataframe to find the entry for article_id = 9724944, 9771903
article_entry = df_articles.filter(pl.col("article_id") == 9771903)

# Display the result
# Filter the df_articles dataframe to find the entry for article_id = 9724944

# Display the result, including the published_time column
if not article_entry.is_empty():
    print("Article Entry:")
    print(article_entry)
    print("Published Time:", article_entry["published_time"].to_list()[0])
else:
    print("No entry found for article_id 9724944.")


published_time = dt.datetime.strptime("2023-05-18 20:45:51", "%Y-%m-%d %H:%M:%S")
impression_time = dt.datetime.strptime("2023-05-18 20:39:25", "%Y-%m-%d %H:%M:%S")
delta = (impression_time - published_time).total_seconds() / 60
