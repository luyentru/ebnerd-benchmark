...
class NRMSModel:
    ...
    # Inputs for user history and candidate articles
        ...
        # ------- User time embedding --------
        user_time_emb = tf.keras.layers.TimeDistributed(time2vec_layer)(time_diff_input)
        # => shape: (batch_size, history_size, 1, 50) before squeeze
        #user_time_emb = tf.squeeze(user_time_emb, axis=-2)  # => (batch_size, history_size, 50)

        # We reduce across the history dimension to get 1 vector per user
        # e.g., average or sum; let's do mean
        #user_time_emb_agg = tf.reduce_mean(user_time_emb, axis=1)
        # => shape: (batch_size, 50)
          # Instead of just averaging, apply attention to user_time_emb
    # Self-attention over time embeddings
        time_att = SelfAttention(
            self.hparams.head_num, self.hparams.head_dim, seed=self.seed
        )([user_time_emb, user_time_emb, user_time_emb])
        
        user_time_emb_agg = AttLayer2(self.hparams.attention_hidden_dim, seed=self.seed)(time_att)
        print("~~~~~~~~~~~user_present shape:", user_present.shape)
        print("~~~~~~~~~~~user_time_emb_agg shape:", user_time_emb_agg.shape)


        # 3) Concatenate the user_present_no_time with the aggregated time embedding
        user_present = tf.concat([user_present, user_time_emb_agg], axis=-1)
        # => shape: (batch_size, attention_hidden_dim + 50)

        # We can optionally pass this user_present through a Dense if we want:
        user_present = Dense(400, activation='relu')(user_present)

        # ------- Candidate time embedding --------
